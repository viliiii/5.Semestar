...
public interface Collection<T> {
    ...
    default <U> void addModified(Collection<? extends U> other, Funkcija<? super U, ? extends T> funkcija){

        other.forEach(new Processor<U>() {
            @Override
            public void process(U value) {
                T result = funkcija.apply(value);
                add(result);
            }
        });
    }

    default void transferInto(Collection<? super T> other, Predicate<? super T> predicate){


        forEach((v) -> {
            if(predicate.test(v)) other.add(v);
        });
